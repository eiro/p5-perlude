...
problems and dreamed to use a linux version of powershell since many years.
The reason is perl has no pipe.

Writting and reading haskell programs, i figured out the 3 features which makes pipe a magic operator:

* it's on demand so you write generators without taking carre about end conditions, you write filters with no assomptions on the size of the stream and you get programs that keeps low memory profiles whatever the size of the data sources.
...
Perl have the ability to write generators a very powerfull and simple way. 
Perl have implicity via $_. 
The only missing part is the filtering toolchain, that's where Perlude came in.

take a look on this problem: what is the 10 first elements of the fibonnaci suite that is a multiple of five. First we need a fibonnaci generator. easy in perl: 
...
But you can't. On the other way, that's why Perlude stolen keywords from haskell prelude. You can write: 
...
There are now keywords that doesn't exists in haskell world 
